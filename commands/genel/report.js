...
class Report extends Command {
	...
	async run (message, args, data) {
		...
		if(!repChannel){
			return message.error("genel/report:MISSING_CHANNEL");
		}

		const member = await this.client.resolveMember(args[0], message.guild);
		if(!member){
			return message.error("genel/report:MISSING_USER");
		}

		if(member.id === message.author.id){
			return message.error("genel/report:INVALID_USER");
		}

		const rep = args.slice(1).join(" ");
		if(!rep){
			return message.error("genel/report:MISSING_REASON");
		}

		const embed = new Discord.MessageEmbed()
			.setAuthor(message.translate("genel/report:TITLE", {
		...
		message.success("genel/report:SUCCESS", {
