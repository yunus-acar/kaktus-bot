...
class Suggest extends Command {
	...
	async run (message, args, data) {
		...
		if(!suggChannel){
			return message.error("genel/suggest:MISSING_CHANNEL");
		}

		const sugg = args.join(" ");
		if(!sugg){
			return message.error("genel/suggest:MISSING_CONTENT");
		}

		const embed = new Discord.MessageEmbed()
			.setAuthor(message.translate("genel/suggest:TITLE", {
		...
		message.success("genel/suggest:SUCCESS", {
