...
class Automod extends Command {
	...
	async run (message, args,data) {
		...
		if(!status || (status !== "on" && status !== "off")){
			return message.error("admin/automod:MISSING_STATUS");
		}
		...
		if(status === "on"){
			...
			data.guild.save();
			message.success("admin/automod:ENABLED", {
			...
			if(message.mentions.channels.filter((ch) => ch.type === "text" && ch.guild.id === message.guild.id).first()){
				...
				data.guild.markModified("plugins.automod");
				data.guild.save();
				message.success("admin/automod:DISABLED_CHANNEL", {
				...
				data.guild.markModified("plugins.automod");
				data.guild.save();
				message.success("admin/automod:DISABLED");
			}
		}
	}
