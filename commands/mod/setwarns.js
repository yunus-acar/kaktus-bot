...
class Setwarns extends Command {
	...
	async run (message, args, data) {
		...
		if(!sanction || (sanction !== "kick" && sanction !== "ban")){
			return message.error("mod/setwarns:MISSING_TYPE");
		}
		...
		if(number === "reset"){
			if(sanction === "kick"){
				...
				data.guild.save();
				return message.success("mod/setwarns:SUCCESS_KICK_RESET", {
			...
			if(sanction === "ban"){
				...
				data.guild.markModified("plugins.warnsSanctions");
				data.guild.save();
				return message.success("mod/setwarns:SUCCESS_BAN_RESET", {
					prefix: data.guild.prefix,
					count: number
				});
			}
		...
		if(sanction === "kick"){
			...
			data.guild.markModified("plugins.warnsSanctions");
			data.guild.save();
			return message.success("mod/setwarns:SUCCESS_KICK", {
				prefix: data.guild.prefix,
				count: number
			});
		}
		...
		if(sanction === "ban"){
			...
			data.guild.markModified("plugins.warnsSanctions");
			data.guild.save();
			return message.success("mod/setwarns:SUCCESS_BAN", {
				prefix: data.guild.prefix,
				count: number
			});
		}
