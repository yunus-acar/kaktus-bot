...
class Ban extends Command {
	...
	async run (message, args, data) {
		...
		if(!user){
			return message.error("mod/ban:MISSING_MEMBER");
		}
		...
		if(banned.some((m) => m.user.id === user.id)){
			return message.error("mod/ban:ALREADY_BANNED", {
				username: user.tag
			});
		}
		...
		if(member){
			...
			if(!member.bannable) {
				return message.error("mod/ban:MISSING_PERM");
			}
		}
        
		await user.send(message.translate("mod/ban:BANNED_DM", {
			username: user.tag,
		...
		message.guild.members.ban(user, { reason } ).then(() => {
			...
			if(data.guild.plugins.modlogs){
				...
				const embed = new Discord.MessageEmbed()
					.setAuthor(message.translate("mod/ban:CASE", {
