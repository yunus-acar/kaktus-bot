	...
	async run (message) {
		...
		if(message.guild){
				...
				if(channelSlowmode){
					...
					if(uSlowmode){
						if(uSlowmode.time > Date.now()){
							...
							return message.author.send(message.translate("admin/slowmode:PLEASE_WAIT", {
								time: delay,
			...
			if(data.guild.plugins.automod.enabled && !data.guild.plugins.automod.ignored.includes(message.channel.id)){
				if(/(discord\.(gg|io|me|li)\/.+|discordapp\.com\/invite\/.+)/i.test(message.content)){
					if(!message.channel.permissionsFor(message.member).has("MANAGE_MESSAGES")){
						...
						return message.error("admin/automod:DELETED", {
			...
			if(afkReason){
				...
				message.sendT("genel/setafk:DELETED", {
					username: message.author.username
				});
			}
			...
			message.mentions.users.forEach(async (u) => {
				...
				if(userData.afk){
					message.error("genel/setafk:IS_AFK", {
						user: u.tag,
						reason: userData.afk
					});
				}
