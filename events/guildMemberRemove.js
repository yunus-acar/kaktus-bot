		...
		if(guildData.plugins.goodbye.enabled){
			...
			if(channel){
				...
				if(guildData.plugins.goodbye.withImage){
					...
					// Background language"
					const background = await Canvas.loadImage("./assets/img/kaktus-Bg.png");
					...
					// Draw server name
					ctx.font = applyText(canvas, member.guild.translate("admin/goodbye:IMG_GOODBYE", {
						server: member.guild.name
					}), 53);
					ctx.fillText(member.guild.translate("admin/goodbye:IMG_GOODBYE", {
						server: member.guild.name
					...
					// Draw number
					ctx.font = "22px Bold";
					ctx.fillText(member.guild.translate("admin/goodbye:IMG_NB", {
					...
					ctx.lineWidth = 15;
					ctx.strokeText(member.guild.translate("admin/goodbye:TITLE"), canvas.width - 620, canvas.height - 330);
					...
					ctx.fillText(member.guild.translate("admin/goodbye:TITLE"), canvas.width - 620, canvas.height - 330);
